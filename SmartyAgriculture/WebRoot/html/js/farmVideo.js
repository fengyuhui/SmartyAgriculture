...
function setVideoDiv() {
    ...
    // if (navigator.appName == "webkit") {
    //     alert(navigator.appName);
    //     alert("请使用 IE 浏览器版本不低于 IE11 的浏览器进行播放。")
    ...
    if (-1 == WebVideoCtrl.I_CheckPluginInstall()) {
        ...
        alert("实时视频播放插件未安装，请安装 WebComponents.exe 视频流播放器插件！");
        // window.location.assign("https://www.baidu.com/s?wd=webcomponents.exe%E6%8F%92%E4%BB%B6&rsv_spt=1&rsv_iqid=0xfbddc8a300014681&issp=1&f=3&rsv_bp=0&rsv_idx=2&ie=utf-8&tn=baiduhome_pg&rsv_enter=1&rsv_sug3=8&rsv_sug1=4&rsv_sug7=100&rsv_t=79e2ud%2B08V5L5qKssdKFIBt0gbJ%2BtjuBiIFYaU7hVqIvvbmWVXN25EX%2FRXZZAfXkE9dBRznj%2BWI&rsv_sug2=0&prefixsug=web%3Comp&rsp=1&inputT=3631&rsv_sug4=3776");
    ...
    WebVideoCtrl.I_InitPlugin(500, 300, {
        iWndowType: 1 //设置窗口全部界面只显示一个全屏视频流
    ...
    for (var index = 1; index < 4; index++) {
        WebVideoCtrl.I_InsertOBJECTPlugin("divPlugin" + index);
    }
    ...
    if (-1 == WebVideoCtrl.I_CheckPluginVersion()) {
        ...
        alert("请检测最新版本的视频播放插件，请双击 WebComponents.exe 更新！");
        return;
    }
}
...
$(function() {
    ...
    $.ajax({
        type: "post",
        url: "http://127.0.0.1:8080/SmartyAgriculture/interface/camera/getCameraList",
        data: {
            pageNum: 1,
            pageSize: 10
        },
...
function Login(data) {
    ...
    var szIP = datacontent[0].ip,
        szPort = datacontent[0].port,
        szUsername = datacontent[0].name,
        szPassword = datacontent[0].passwd;
    // alert("szip:" + szIP + ",szPort:" + szPort + ",szUsername:" + szUsername + ",szPassword:" + szPassword + ".");
    if ("" == szIP || "" == szPort) {
        return;
    }
    // setVideoDiv();
...
function getChannelInfo(ip) {
    ...
    WebVideoCtrl.I_GetDigitalChannelInfo(szIP, {
        ...
        success: function(xmlDoc) {
            ...
            //解析ip地址里头有多少个摄像头视频流（参数id为摄像头的个数）
            $.each(oChannels, function(i) {
                var id = parseInt($(this).find("id").eq(0).text(), 10),
                    name = $(this).find("name").eq(0).text(),
                    online = $(this).find("online").eq(0).text();
                // alert("id :" + typeof(id) + ": " + id);
                // alert("name :" + name);
                // alert("online :" + online);
                if ("false" == online) { // filter the forbidden IP channel //过滤禁用的ip通道
                    return true;
                }
                if ("" == name) {
                    name = "IPCamera " + ((id - nAnalogChannel) < 12 ? "0" + (id - nAnalogChannel) : (id - nAnalogChannel));
                }
                oSel.append("<option value='" + id + "' bZero='false'>" + name + "</option>");
                // alert("id:" + id);
                // alert("divPlugin" + id);
                ...
                //     iWndowType: 1 //设置窗口全部界面只显示一个全屏视频流
                // });
                // WebVideoCtrl.I_InsertOBJECTPlugin("divPlugin" + id);
                clickStartRealPlay(szIP, id);
            });
            // console.log(szIP + " 获取IP通道成功！");
            // clickStartRealPlay();
        },
        error: function() {
            alert(szIP + " 获取IP通道失败！");
...
// strat real play 开始播放
function clickStartRealPlay(ip, id) {
    ...
    // alert("iChannelID :" + iChannelID); //更改播放第几个视频流的数据
...
// display callback info 展示回调信息
// function showCBInfo(szInfo) {
//     szInfo = "<div>" + dateFormat(new Date(), "yyyy-MM-dd hh:mm:ss") + " " + szInfo + "</div>";
//     $("#cbinfo").html(szInfo + $("#cbinfo").html());
// }

// get window size 获得窗口大小
