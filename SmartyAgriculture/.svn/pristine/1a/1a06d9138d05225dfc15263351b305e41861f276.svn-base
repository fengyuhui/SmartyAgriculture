	...
	<div class="page-content">
		<div class="page-header">
			...
			<a href="" role="button" class="green"
				...
				审核商品 <small> <i class="icon-double-angle-right"></i>
				</small>
			</h1>
		</div>
		<!-- /.page-header -->
							...
							<div class="profile-info-value" >
								...
								<div class="profile-info-row">
									...
									<div class="profile-info-value">
										...
										<div> <input id="submit" type="button" value="确  定" style="margin-left: 35px;
    background-color: #438eb9;
    font-size: 18px;
    margin-top: 10px;color:white;" onclick="updateAuditStatus()" data-id="${goodsList.id}"/></div>
									</div>
								</div>
							</div>
...
function updateAuditStatus(){
	...
	if(auditStatus==undefined){
		...
		var url="productAudit/updateAuditStatus/"+id+"/"+auditStatus;
		$.ajax({ 
	        type:"POST", 
	        url:url,
	    	dataType : "json",
			...
			success: function(data){  
			   alert("审核成功！");
			   setTimeout(function(){window.location.href="productAudit/auditIndex/0/1/prvious";},3000);
	        },  
	        error: function()   
	        {  
	            alert("审核失败！");
