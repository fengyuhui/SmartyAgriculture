					...
					<div class="row">
						<div class="col-xs-12">
							...
							<a role="button" class="green"
									data-toggle="modal"style="color: #438eb9!important;font-size: 26px;"> 信息审核列表 </a>/
									
								
								<a href="<%=request.getContextPath()%>/farmVisits/index/0/1/prvious/" role="button" class="green"
									data-toggle="modal" > 参观项目列表 </a>
													...
													<tr>
														...
														<td>
															<div
																...
																<!-- 查看商品信息操作 -->
																<a class="blue"
																	href="farmVisits/typeInfo/${item.id}"> <i
																	class="icon-zoom-in bigger-130"></i>
																</a>
																
																<c:if test="${item.auditStatus == 1}">
                                                                <!-- 确认信息发布 -->
                                                                <a class="green" href="farmVisitsAudit/acceptAdd/${item.id}">
                                                                          <i class="icon-check bigger-130"></i>
                                                                </a>              
                                                                </c:if>
                                                                <c:if test="${item.auditStatus == 2}">
                                                                <!-- 确认草稿信息操作 -->
                                                                <a class="green" href="farmMessage/judgeDraft?id=${item.id}">
                                                                          <i class="icon-check bigger-130"></i>
                                                                </a>              
                                                                </c:if>
                                                                <c:if test="${item.auditStatus == 4}">
                                                                <!-- 确认删除信息删除成功操作 -->
                                                                <a class="green" href="farmVisitsAudit/delete/${item.id}">
                                                                          <i class="icon-check bigger-130"></i>
                                                                </a>                
                                                                </c:if>	
                                                                <!-- 真实删除草稿和新增未审核项目 -->
                                                                ...
                                                                <!-- 撤销删除 -->
                                                                <c:if test="${item.auditStatus == 4}">
                                                                <a class="green" href="farmVisitsAudit/cancelDeleteRequest/${item.id}">
                                                                          <i class="icon-remove bigger-130"></i>
                                                                </a>
                                                                </c:if>
