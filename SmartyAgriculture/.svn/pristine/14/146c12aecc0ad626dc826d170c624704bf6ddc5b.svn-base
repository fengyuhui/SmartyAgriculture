...
<body>
	<div class="page-content">
		<div class="page-header">
			<h1>
				编辑管理员信息 <small> <i class="icon-double-angle-right"></i>
				</small>
			</h1>
		</div>
		<!-- /.page-header -->
					...
					<div class="form-group">
						...
						<div class="col-sm-9">
							<input type="password" class="col-xs-10 col-sm-5"
								id="form-input-readonly" name="passwd" value="" name="description" style="width:25%"/>							
						</div>
					</div>
						...
						<div class="col-sm-9">					
						 <c:forEach items="${authList}" varStatus="i" var="item" >  
							<c:if test="${item.type==1}">       
								<input type="checkbox" name="autherList[]" 
									<c:if test="${hasAuth[item.id] == true}">
									checked="checked"
									</c:if>
